...
"  Zip files in subdirectories also:            zip -r z *",
"  Escape wildcards so -r works (on unix):      zip -r z '*'",
...
/* strange but true: if I delete this and put these two endifs adjacent to
  ...
  if (noisy && bad_files_so_far)
  {
    ...
    WriteNumString(bytes_total - bad_bytes_so_far, tempstrg);
